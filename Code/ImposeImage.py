...
#out = cv2.VideoWriter('Imposed.mp4',cv2.VideoWriter_fourcc(*'mp4v'), 30, (640,480))
...
def read_img_and_video_and_get_coordinates():
    ...
    while(True):
        ...
        for i in contour:
            ...
            if area<2200 and area>50 and len(approx) == 4:
            ...
            # use the predicted value if we have no measurement
            if len(all_tags) == 0:
                all_tags.append(est)
    ...
    performance_boost = ((count_kf - count_og)/count_og) * 100
    print("Performance gain by using Kalman Filter: ", performance_boost)
    #out.release()
    vid.release()
    return
